...
<div class="rounded-lg border p-4 mb-6" style="background:var(--color-surface); border-color:var(--color-border-soft);">
    ...
    <form method="POST" action="{{ route('trading.accounts.store') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        @csrf
        <div>
        ...
        @endif
        <div>
            <label class="block text-[11px] mb-1" style="color:var(--color-text-muted);">Nombre de la cuenta</label>
            <input type="text" name="label" placeholder="Ej: Mi cuenta principal" value="{{ old('label') }}"
        ...
        <div class="flex items-end">
            <button type="submit" onclick="showSaving()"
    ...
    <p class="text-[10px] mt-2" style="color:var(--color-text-muted);">Las credenciales se almacenan cifradas y nunca se muestran de nuevo.</p>
</div>
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b" style="border-color:var(--color-border-soft);">
            <div class="flex items-center gap-3">
                <div>
                    ...
                    <p class="text-[11px]" style="color:var(--color-text-muted);">{{ ucfirst($account->broker) }}</p>
                </div>
...
{{-- Modal: seleccionar estrategia para suscribir --}}
        ...
        <div class="p-4">
            @if ($availableConfigs->isEmpty())
                <p class="text-sm text-center py-4" style="color:var(--color-text-muted);">
                    No hay estrategias activas en Paper Trading. El admin debe activar al menos una desde Backtesting.
                </p>
            @else
                <p class="text-[11px] mb-3" style="color:var(--color-text-muted);">
                    Solo se muestran las estrategias activas en Paper Trading. Los parámetros son los mismos que usa el sistema en producción.
                </p>
                <table class="w-full text-[11px]" style="color:var(--color-text-muted);">
                    <thead>
                        <tr class="border-b" style="border-color:var(--color-border-soft);">
                            ...
                            <tr class="border-b" style="border-color:var(--color-border-soft);">
                                ...
                                <td class="py-2 px-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form method="POST" id="subForm_{{ $config->id }}" action="" class="inline">
                                            @csrf
                                            <input type="hidden" name="paper_strategy_config_id" value="{{ $config->id }}">
                                            <button type="button"
                                                    onclick="confirmSubscribe({{ $config->id }}, '{{ $config->display_name }}')"
...
{{-- Modal: ver configuración completa --}}
...
function openSubscribeModal(accountId, accountLabel) {
    ...
    // Actualizar el action de cada form con el accountId correcto
    document.querySelectorAll('[id^="subForm_"]').forEach(form => {
        form.action = `/trading/accounts/${accountId}/subscriptions`;
    });
...
function confirmSubscribe(configId, configName) {
    Swal.fire({
        title: 'Suscribir estrategia',
        html: `¿Suscribir <b>${configName}</b> a esta cuenta?<br><br>
               El bot comenzará a operar con esta estrategia en cuanto haya una señal válida.`,
        icon: 'question',
        ...
        if (result.isConfirmed) {
            ...
            document.getElementById(`subForm_${configId}`).submit();
        }
    });
}
...
function showConfigModal(params, title) {
    ...
    const labels = {
        ...
        mode: 'Modo', regime_filter: 'Filtro de régimen', macro_trend_filter: 'Filtro macro H4',
    ...
    document.getElementById('configModalBody').innerHTML = html || '<p>Sin parámetros disponibles.</p>';
...
function showSaving() {
    Swal.fire({ title: 'Guardando cuenta...', allowOutsideClick: false, background: '#11161F', color: '#E5E9F0', didOpen: () => Swal.showLoading(), timer: 4000 });
}
...
function confirmAccountToggle(event, label, isActive) {
    ...
    Swal.fire({
        ...
        html: isActive
            ? `¿Pausar <b>${label}</b>? Todas sus suscripciones se pausarán también.`
            : `¿Activar <b>${label}</b>?`,
        icon: 'warning', showCancelButton: true,
        confirmButtonText: isActive ? 'Pausar' : 'Activar',
        cancelButtonText: 'Cancelar',
...
function confirmDelete(event, label) {
    ...
    Swal.fire({
        ...
        html: `¿Eliminar <b>${label}</b>? Esta acción no se puede deshacer.`,
        icon: 'warning', showCancelButton: true,
...
function confirmSubToggle(event, name, isActive) {
    ...
    Swal.fire({
        ...
        html: isActive ? `¿Pausar <b>${name}</b>? Dejará de operar en esta cuenta.` : `¿Activar <b>${name}</b>?`,
        icon: 'warning', showCancelButton: true,
...
function confirmSubDelete(event, name) {
    ...
    const form = event.target;
    Swal.fire({
        title: 'Quitar estrategia',
        html: `¿Quitar <b>${name}</b> de esta cuenta?`,
        icon: 'warning', showCancelButton: true,
